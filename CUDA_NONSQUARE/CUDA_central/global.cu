...
#include "Soft.h"
__global__
void random_generate(curandStateMtgp32 (*state)[NCURAND]){
  unsigned int n_parameter_set = (blockIdx.x + blockIdx.y * (int)GRIDL) % 200;
  unsigned int n_seed = (int)((float)(blockIdx.x + blockIdx.y * (int)GRIDL) / 200.0);
...
__global__
void JUST_MOVE_f(float *rrx1, float *rry1, short *nnparticle1, float *rrx2, float *rry2, short *nnparticle2){
  unsigned int index =
    ((blockIdx.x * MBLOCKL + threadIdx.x) % 2 == 0 ?
     (blockIdx.x * MBLOCKL + threadIdx.x) / 2 : (blockIdx.x * MBLOCKL + threadIdx.x  + Nblock) / 2);
  ...
  if(nnparticle1[indey * Nblock + index] >= nmax){
    printf("Error on (%d, %d), %d\n", index, indey, nnparticle1[indey * Nblock + index]);
    assert(nnparticle1[indey * Nblock + index] < nmax);
  }

  for(int i = 0; i < nnparticle1[indey * Nblock + index]; i++){
    rrx2[(i * Nblock + indey) * Nblock + index] = rrx1[(i * Nblock + indey) * Nblock + index];
    rry2[(i * Nblock + indey) * Nblock + index] = rry1[(i * Nblock + indey) * Nblock + index];
  }

...
__global__ void shift_checkerboard(float *rrx, float *rry, short *nnparticle, float rshift, int direction, double LLblock)
{
  
  __shared__ float temp_rx[SHIFT_BLOCKL * SHIFT_BLOCKL * nmax], temp_ry[SHIFT_BLOCKL * SHIFT_BLOCKL * nmax];
  ...
  double LLblocky = LLblock * sqrt(3.0) * 0.5;
  
  for(short color = 0; color < 4; color++){
    unsigned int xxx = (color % 2 == 0 ? SHIFT_BLOCKL * blockIdx.x + 2 * threadIdx.x : SHIFT_BLOCKL * blockIdx.x + 2 * threadIdx.x + 1);
    unsigned int yyy = (color < 2 ? SHIFT_BLOCKL * blockIdx.y + 2 * threadIdx.y : SHIFT_BLOCKL * blockIdx.y + 2 * threadIdx.y + 1);
    unsigned int index = (2 * threadIdx.y + (color < 2 ? 0 : 1)) * SHIFT_BLOCKL + (2 * threadIdx.x + (color % 2 == 0 ? 0 : 1));
    shifted_nparticle[index] = 0;

    unsigned int location = yyy * Nblock + (xxx % 2 == 0 ? xxx / 2 : (xxx + Nblock) / 2);

    for(int k = 0; k < tex1Dfetch(read_nparticle, location); k++){
      float ppx = tex1Dfetch(read_rx, k * Nblock * Nblock + location) + rshift * (float)a;
      float ppy = tex1Dfetch(read_ry, k * Nblock * Nblock + location) + rshift * (float)b;

      short vecx = (direction % 2 == 0 ? (ppx > 0 && ppx < LLblock ? 0 : 1) : ((ppy > 0 && ppy < LLblocky ? 0 : 1)));
    ...
    location = ((yyy - b + Nblock) % Nblock) * Nblock + (((xxx - a + Nblock) % Nblock) % 2 == 0 ? ((xxx - a + Nblock) % Nblock) / 2 : ((xxx - a + Nblock) % Nblock + Nblock) / 2);
    ...
    for(int k = 0; k < tex1Dfetch(read_nparticle, location); k++){
      ...
      if(vecx == 1){
      	...
      	temp_ry[j] = ppy - LLblocky * (float)b;
      	shifted_nparticle[index]++;
      }
    }

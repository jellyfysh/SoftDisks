  ...
  for(int x = 0; x < Nblock; x++){
    for(int y = 0; y < Nblock; y++){
      ...
      for(int k = 0; k < nparticle(x, y); k++){
      	int vecx = (int)((rx(x, y, k) + offsetx) / Lblockx + 1.0) - 1;
      	int vecy = (int)((ry(x, y, k) + offsety) / Lblocky + 1.0) - 1;

      	temp_rx((x + vecx + Nblock) % Nblock, (y + vecy + Nblock) % Nblock, temp_nparticle((x + vecx + Nblock) % Nblock, (y + vecy + Nblock) % Nblock)) 
          = rx(x, y, k) + offsetx - vecx * Lblockx;
      	temp_ry((x + vecx + Nblock) % Nblock, (y + vecy + Nblock) % Nblock, temp_nparticle((x + vecx + Nblock) % Nblock, (y + vecy + Nblock) % Nblock)) 
          = ry(x, y, k) + offsety - vecy * Lblocky;
      	temp_nparticle((x + vecx + Nblock) % Nblock, (y + vecy + Nblock) % Nblock)++;

...
void measure_e_p(double & outpute, double & outputp, double & outputhv){
  ...
  for(int i = 0; i < Nblock; i++){
	...
	for(int x = -1; x <= 1; x++){
	  for(int y = -1; y <= 1; y++){
	    for(int m = 0; m < nparticle((i + x + Nblock) % Nblock, (j + y + Nblock) % Nblock); m++){
	      float x0 = rx(i, j, k);
	      float y0 = ry(i, j, k);
	      float x1 = rx((i + x + Nblock) % Nblock, (j + y + Nblock) % Nblock, m) + (float)x * Lblock;
	      float y1 = ry((i + x + Nblock) % Nblock, (j + y + Nblock) % Nblock, m) + (float)y * Lblock;

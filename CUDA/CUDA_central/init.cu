...
void Set_Mersenne_Twister_GPU(int input) {
  ...
  cudaMalloc((void **)&devMTGPStates,
             NCURAND * n_seed * sizeof(curandStateMtgp32));
  cudaMalloc((void **)&devKernelParams, sizeof(mtgp32_kernel_params));
  curandMakeMTGP32Constants(mtgp32dc_params_fast_11213, devKernelParams);
  // This is to let each block have a different seed for Mersenne Twister
  for (int i = 0; i < n_seed; i++)
    curandMakeMTGP32KernelState(devMTGPStates[i], mtgp32dc_params_fast_11213,
                                devKernelParams, NCURAND,
                                seed * (input + 1) * (i + 1));
}
...
void Set_init_conf(void) {
  ...
  for (int j = 0; j < Nblock * Nblock; j++)
    nparticle[j] = 0;
  for (int j = 0; j < nmax * Nblock * Nblock; j++)
    rx[j] = DUMMY_OFFSET, ry[j] = DUMMY_OFFSET;
...
void random_config(void) {
  ...
  int count = 0;
  int comptrNx = (int)sqrtf(sqrtf(3.0) * 0.5 * NNN) + 1;
  int comptrNy = (int)((float)NNN / (float)comptrNx) + 1;
  ...
  for (int y = 0; y < comptrNy; y++) {
    for (int x = 0; x < comptrNx; x++) {
      if (count < NNN) {
        float px =
            comptrspx * ((float)x + (float)(y % 2) * 0.5) + 0.1 * comptrspx;
        float py = comptrspx * y * sqrtf(3.0) * 0.5 + 0.1 * comptrspx;
        while (px < 0)
          px += Lbox;
        while (py < 0)
          py += Lbox;
        ...
        while (py > Lbox)
          py -= Lbox;
        int index = (int)(px / Lblock);
        int indey = (int)(py / Lblock);

        rx(index, indey, nparticle(index, indey)) = px - (float)index * Lblock;
        ry(index, indey, nparticle(index, indey)) = py - (float)indey * Lblock;

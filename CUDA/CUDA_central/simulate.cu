...
void simulate(float BBETA) {
  ...
  dim3 block(BLOCKL / 2,
             BLOCKL / 2); // Activate BLOCKL/2 \times BLOCKL/2 threads

  dim3 Sgrid(SHIFT_GRIDL, SHIFT_GRIDL); // Activate GRIDL \times GRIDL blocks
  dim3 Sblock(SHIFT_BLOCKL / 2,
              SHIFT_BLOCKL / 2); // Activate BLOCKL/2 \times BLOCKL/2 threads
  ...
  // The loop below corresponds to 4x4x10 Monte Carlo sweeps
  for (int k = 0; k < 10; k++) {
    short direction = (int)(4.0 * genrand(mt));
    float rshift = 0.5 * Lblock * (float)genrand(mt);

    cudaBindTexture(0, read_rx, rx, cudaCreateChannelDesc<float>(),
                    sizeof(float) * nmax * Nblock * Nblock);
    cudaBindTexture(0, read_ry, ry, cudaCreateChannelDesc<float>(),
                    sizeof(float) * nmax * Nblock * Nblock);
    cudaBindTexture(0, read_nparticle, nparticle,
                    cudaCreateChannelDesc<short>(),
                    sizeof(short) * Nblock * Nblock);
    shift_checkerboard<<<Sgrid, Sblock>>>(devrx, devry, devnparticle, rshift,
                                          direction, Lblock);
    cudaDeviceSynchronize();
    JUST_MOVE_f<<<Mgrid, Mblock>>>(devrx, devry, devnparticle, rx, ry,
                                   nparticle);
    ...
    rshiftx +=
        (direction % 2 == 0 ? (direction < 2 ? 1.0 : -1.0) : 0.0) * rshift;
    rshifty +=
        (direction % 2 == 1 ? (direction < 2 ? 1.0 : -1.0) : 0.0) * rshift;
    ...
    cudaBindTexture(0, read_nparticle, nparticle,
                    cudaCreateChannelDesc<short>(),
                    sizeof(short) * Nblock * Nblock);
    // The loop below corresponds to 4x4 Monte Carlo sweeps
    for (int m = 0; m < 4; m++) {
      // One call of update_Metropolis corresponds to 4 Monte Carlo sweeps
      update_Metropolis<<<grid, block>>>(rx, ry, color[m], rshiftx, rshifty,
                                         Lblock, Lbox, BBETA, devMTGPStates);
    }

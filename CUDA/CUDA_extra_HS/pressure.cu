    ...
    for (int y_index = 0; y_index < Nblock; y_index++) {
      for (int p_index = 0; p_index < nparticle(x_index, y_index); p_index++) {
        ...
        while (temp_chain_length_x > cdistance[index][0]) {
          ...
          index = cindex[index][2] * Nblock * Nblock +
                  cindex[index][1] * Nblock + cindex[index][0];
        }
        ...
        while (temp_chain_length_y > cdistance[index][2]) {
          ...
          index = cindex[index][5] * Nblock * Nblock +
                  cindex[index][4] * Nblock + cindex[index][3];
        }
...
void find_first_collision(vector<vector<int>> &cindex,
                          vector<vector<double>> &cdistance) {

  for (int x_index = 0; x_index < Nblock; x_index++) {
    for (int y_index = 0; y_index < Nblock; y_index++) {
      for (int p_index = 0; p_index < nparticle(x_index, y_index); p_index++) {
        int next_p_index = p_index, next_x_index = x_index,
            next_y_index = y_index;
        int next_p_indey = p_index, next_x_indey = x_index,
            next_y_indey = y_index;

        double x0 = rx(x_index, y_index, p_index);
        double y0 = ry(x_index, y_index, p_index);

        double min_dist_x = 10.0 * Lblock, min_dist_y = 10.0 * Lblock;
        double excess_x, excess_y;
        bool flag_found_x = false, flag_found_y = false;

        // Find a candidate within the same cell to which the moving particle
        // collides
        ...
        // Find a candidate to which the moving particle collides in difference
        // cells
        ...
        { // in the x direction
          ...
          do {
            for (int y = -1; y <= 1; y++) {
              if (x != 0 || y != 0) {
                for (int m = 0; m < nparticle((x_index + x + Nblock) % Nblock,
                                              (y_index + y + Nblock) % Nblock);
                     m++) {
                  double x1 = rx((x_index + x + Nblock) % Nblock,
                                 (y_index + y + Nblock) % Nblock, m) +
                              (double)x * Lblock;
                  double y1 = ry((x_index + x + Nblock) % Nblock,
                                 (y_index + y + Nblock) % Nblock, m) +
                              (double)y * Lblock;
          ...
          int index = p_index * Nblock * Nblock + y_index * Nblock + x_index;
          cindex[index][0] = next_x_index, cindex[index][1] = next_y_index,
          cindex[index][2] = next_p_index;
        ...
        { // in the y direction
          ...
          int index = p_index * Nblock * Nblock + y_index * Nblock + x_index;
          cindex[index][3] = next_x_indey, cindex[index][4] = next_y_indey,
          cindex[index][5] = next_p_indey;
...
void move_block(int x_index, int y_index, int p_index) {
  ...
  for (int i = 0; i < nparticle(target_index, y_index) - 1; i++) {
    double dist =
        (rx(target_index, y_index, i) -
         rx(target_index, y_index, nparticle(target_index, y_index))) *
            (rx(target_index, y_index, i) -
             rx(target_index, y_index, nparticle(target_index, y_index))) +
        (ry(target_index, y_index, i) -
         ry(target_index, y_index, nparticle(target_index, y_index))) *
            (ry(target_index, y_index, i) -
             ry(target_index, y_index, nparticle(target_index, y_index)));
    if (dist < 1.0) {
      cerr << rx(x_index, y_index, p_index) << " " << num_block << " " << Lblock
           << endl;
    }
  }
}

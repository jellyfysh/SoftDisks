  ...
  for (int k = 0; k < nloop; k++) {
    ...
    for (unsigned int j = 0; j < temp_nparticle; j++) {
      if (j != pindex) {
        ...
        float predist = (prex - temp_rx[(nmax * index + j)]) *
                            (prex - temp_rx[(nmax * index + j)]) +
                        (prey - temp_ry[(nmax * index + j)]) *
                            (prey - temp_ry[(nmax * index + j)]);

        float nexE = (nexdist < sq_cutoff
                          ? powq(nexdist, -rpotential * 0.5) - shift_cutoff
                          : 0);
        float preE = (predist < sq_cutoff
                          ? powq(predist, -rpotential * 0.5) - shift_cutoff
                          : 0);
        difE += nexE - preE;
      }
    }
    ...
    for (unsigned int j = 0; j < nn_block; j++) {
      ...
      float nexE =
          (nexdist < sq_cutoff ? powq(nexdist, -rpotential * 0.5) - shift_cutoff
                               : 0);
      float preE =
          (predist < sq_cutoff ? powq(predist, -rpotential * 0.5) - shift_cutoff
                               : 0);
      difE += nexE - preE;
    }
    //=== 3rd block ===//
    ...
    if (curand_uniform(&state[n_seed][n_parameter_set]) < pro)
      judge = 1;
    else
      judge = 0;
